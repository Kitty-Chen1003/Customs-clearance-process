...
from utils.parse_zc429_xml import parse_zc429_xml
...
class MultiFileUploadDialog(QDialog):
    ...
    def get_mrn(self):
        QMessageBox.information(self, "Get Clearances MRN", "Not Support yet.")
        self.accept()
...
class OldmultiFileUploadDialog(QDialog):
    ...
    def get_mrn(self):
        ...
        try:
            ...
            # 写入 MRN(CLEARANCES) 列
            df["MRN(CLEARANCES)"] = df["TrackingNumber"].apply(lambda x: ucr_to_mrn.get(str(x).strip(), ""))
...
class AddItemsClearancesLSAS(QWidget):
    ...
    def __init__(self, api):
        ...
        self.clearance_data = []  # [{ process_file: str, zc429_files: []}]
    ...
    def build_title(self):
        # 标题
        title = QLabel("AddItemsClearances")
    ...
    def build_process_clearance_panel(self):
        ...
        process_429_group.setLayout(process_clearance_layout)
        ...
        process_clearance_layout.setStretchFactor(left_layout, 1)
        process_clearance_layout.setStretchFactor(right_layout, 1)
    ...
    def build_action_buttons(self):
        ...
        self.get_mrn_button = QPushButton("Get Clearance MRN")
        self.get_mrn_button.setFont(QFont("Microsoft YaHei", 12))
        self.get_mrn_button.setStyleSheet("padding: 10px;")
        self.get_mrn_button.clicked.connect(self.get_clearance_mrn)
        ...
        self.save_button = QPushButton("Save_Clearance")
        self.save_button.setFont(QFont("Microsoft YaHei", 12))
        self.save_button.setStyleSheet("padding: 10px;")
        self.save_button.clicked.connect(self.save_clearance)
    ...
    def add_429_file(self):
        ...
        if file_dialog.exec_():
            self.zc429_list.clear()
            self.clearance_data[row]["zc429_files"] = []

            for file_path in file_dialog.selectedFiles():
                self.clearance_data[row]["zc429_files"].append(file_path)
    ...
    def delete_429_file(self):
        ...
        self.clearance_data[row]["zc429_files"].clear()
    ...
    def write_clearance(self):
        ...
        for data in self.clearance_data:
            ...
            try:
                matched, unmatched = self.write_single_process_clearance(
                    process_path,
                    zc429_files
                )
    ...
    def write_single_process_clearance(self, process_path, zc429_files):
        ...
        for file_path in zc429_files:
            try:
                with open(file_path, "rb") as f:
                    all_clearance_info.extend(parse_zc429_xml(f))
            except Exception as e:
                print(f"Error parsing {file_path}: {e}")

        if not all_clearance_info:
            raise ValueError("No valid MRN found in ZC429 files.")

        # 构造 UCR → MRN 映射
        # ucr_to_mrn = dict(all_clearance_info)

        suffix = self.suffix_input.text().strip()

        ucr_to_mrn = {}

        for ucr, mrn in all_clearance_info:
    ...
    def save_clearance(self):
        self.save_box_parcel_clearance_excel(self.json_response)

    def save_box_parcel_clearance_excel(parent_widget, json_response: dict):
        try:
            ...
            df = pd.DataFrame(data, columns=["boxnumber", "parcel", "clearance"])

            default_filename = f"{global_state.flight}_{global_state.carrier}_clearance.xlsx"
